...
クラスを抽象化することで、クラスの物理的な関係（＝ファイルのインクルード関係）を分離し、独立に変更できるようにするデザインパターン。
GoF 本にもある古典的なデザインパターン。

> 目的：抽出されたクラスと実装を分離して、それらを独立に変更できるようにする

(238p, ガイドライン 28)

> ローカルで多態性も持たない形の Bridge パターンを、 Pimpl イディオムと言います。

(243p, ガイドライン 28)

Strategy パターンは実装詳細を外部から設定し、論理的な依存関係を削減している。
そのため振る舞いに関するパターン (behavioral design pattern) に分類される。
一方 Bridge パターンは、実装詳細を内部で設定するが、物理的な（ファイルの）依存関係を分離している。
そのため構造に関するパターン (structural design pattern) に分類される。

(246p, ガイドライン 28)

Bridge パターンの欠点として、継承による抽象化に伴う、性能面のオーバーヘッドがある。
オーバーヘッドは、間接参照、仮想関数のコール、インライン展開ができなくなる、動的メモリ割当、メモリオーバーヘッドの全５種類が挙げられる。

(248p, ガイドライン 28)

Pimpl イディオムはオーバーヘッドがある一方、クラス本体のサイズは小さくすることができる。
これは Visitor パターンなどで `std::variant` を使う場合にはメリットとなる。

(251p, ガイドライン 29)

#### Prototype パターン

基底クラスに virtual な`clone()` を定義し、それをサブクラスで実装することで、型のコピーを抽象化するデザインパターン。

> 目的：生成すべきオブジェクトの種類を原型となるインスタンスを使って明確にし、それをコピーすることで新たなオブジェクトの生成を行う。

(257p, ガイドライン 30)

> Prototype パターンの「現代的 (modern)」な実装というのはありません
> （中略）
> 他のデザインパターンと比較しても値セマンティクスの解は存在しません。

(260p, ガイドライン 30)

型自体を値セマンティクスにすれば不要になるが、参照セマンティクスベースの型のコピーを抽象化する場合は、 Prototype パターンが正しい選択となる。

#### External Polymorphism パターン

型に振る舞いを所有させる (Strategy Pattern) のではなく、
型と振る舞い(Concept)の組みとなるクラス (Model) を外部で新たに定義することで、
型と振る舞いを完全に分離するデザインパターン。

非多態な一連の型の要件（必要な振る舞い）を抽象化する外部階層テンプレートを定義し、そこで要件を実装することで、型と実装詳細とを独立して実装できる。
多態動作全体をバリエーションポイントとして切り出す。
Type Erasure パターンの実装に必要なパターン。

> 目的：非多態（継承関係を持たない、仮想関数を１つも持たない）な型を、多態性を持つものとして扱えるようにする。関係を持たないクラスでも画一的に扱えるようにする。

(264p, ガイドライン 31)

> 大きな短所が１つだけあります。External Polymophism パターンは簡潔で綺麗な解という期待に完全には応えておらず、また値セマンティクスベースの解では決してありません。
> ポインタ、手動によるメモリ割り当て、継承階層を削減しているわけではなく、ユーザーコードの簡略化にも貢献していません。

(273p, ガイドライン 31)

#### Type Erasure パターン

External Polymorphism, Bridge, Prototype パターンを組み合わせ、
型と振る舞いを完全に分離し、その組みを、コピー、ムーブ可能な値型として定義できるデザインパターン。

データと振る舞いの組みを表す型（コードでは `Shape`）は、実際のデータと振る舞いの型を持たず、代わりにそのポインタしか持たない。
...
> 恐らくもっとも大きな短所は、実装の複雑さでしょう。
> （中略）
> 読者が基本実装を卒業し、性能や例外安全などを考慮し始めると、実装詳細はあっという間にとてもトリッキーになります。
> この場合に考えられるもっとも安全で簡便な対処は、すべてを自分で実装せずサードパーティのライブラリを利用することです。

(290p, ガイドライン 32)

> ２つ目の短所は、ずっと重要で大きな制約ですが、（中略）Type Erasure パターンでは二項演算が一筋縄ではいかないのです。

(290p, ガイドライン 32)

#### Decorator パターン

サブクラスの一部にのみ機能を追加したいとき、その追加機能をバリエーションポイントとして切り出し、別のサブクラスとして定義する。

> 目的：オブジェクトに責任を動的に追加する。Decorator パターンは、サブクラス化よりも柔軟な機能拡張方法を提供する。

(330p, ガイドライン 35)

Adapter パターンはインターフェースの変換を行うが、既存機能を変更や拡張することはない。一方 Decorator パターンは既存のインターフェースを維持しつつ、既存機能の変更と拡張を行う。
Strategy パターンは特定機能の実装詳細を外部に分離することで、実装詳細が持つ依存関係を取り除き、複数の実装を切り替えることができる。一方 Decorator パターンは機能同士の結合を取り除くことで、実装詳細どうしの結合部分が持つ依存関係を取り除き、機能を追加、拡張することができる。
より複雑な実装を持つ場合は、Decorator パターンより Strategy パターンを選択する。

(339p, ガイドライン 35)

短所として、Decorator パターンを用いると仮想関数コールによるオーバーヘッドが発生する。
また無意味な Decorator 同士の組み合わせを許してしまうため、実装者が適切に組み合わせる必要がある。

(340p, ガイドライン 35)

#### Singleton パターン

グローバルな状態を抽象化し、そのアクセスを提供する「実装パターン」。

> Singleton パターンは抽象化を導入も表現もしていないのです。
> （中略）
> 依存関係の軽減や管理など、著者が定義するソフトウェア設計の要を目的としていないのです。Singleton パターンが目的にしているのはインスタンス数を１つに制限することです。このため Singleton パターンはデザインパターンではなく実装パターンに過ぎません。
