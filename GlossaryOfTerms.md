...
- e,g., ムーブセマンティクス：「ムーブ」という振る舞いを実行させるための、コードの書き方。
- e.g., 参照セマンティクス/値セマンティクス：コピー（代入、コンストラクタ呼出）操作のコードを書いたとき、それが参照を渡す振る舞いか、真の値コピーを行う振る舞いを意味すること。（どちらの振る舞いになるかを明確にするための、コードの書き方。）
